		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(Reg==null){JOptionPane.showMessageDialog(null, "请输入预约ID");}
					else{
		...
		btnNewButton_8.addActionListener(new ActionListener() {// 预约挂号按钮
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (cn == 0) {
						JOptionPane.showMessageDialog(null, "没有预约信息，不能挂号!");
					} else {
		...
		btnNewButton_3.addActionListener(new ActionListener() {// 挂号按钮
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if(patName==""){JOptionPane.showMessageDialog(null, "患者姓名不能为空!");}
					else if(patgender==""){JOptionPane.showMessageDialog(null, "患者性别不能为空!");}
					else if(Age==""){JOptionPane.showMessageDialog(null, "患者年龄不能为空!");}
					else if(deptName==""){JOptionPane.showMessageDialog(null, "挂号科室不能为空!");}
					else{
					// 存储新病人信息
					String sql1 = "insert into Patient values('" + patName + "','" + patgender + "',?,'" + patTel
							+ "',6)";
					Connection con = SQLOperate.Connection();
					...
					// 通过科室取得医生ID
					int doctID = -1;
					String sql3 = "select DoctorID from Doctor where DoctorDeptID=(select DeptID from Department where DeptName='"
							+ deptName + "')";
					...
					String sql4 = "insert into Registration(PatientID,DoctorID,RegTime,RegState) values(?,?,'" + regtime
							+ "','挂号')";
			...
			public void actionPerformed(ActionEvent e) {// 收费按钮
				{
					try {
						...
						if (cn == 0) {
							JOptionPane.showMessageDialog(null, "没有收费信息，不能收费!");
						} else {
