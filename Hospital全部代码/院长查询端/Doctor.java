	...
	public Doctor() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String sql="select Doctor.DoctorName, COUNT(charge.RegID) as dn,SUM(charge.sumprice) as money  from Doctor left join charge on Doctor.DoctorID=(select DoctorID from Registration where Registration.RegID=charge.RegID)  where Doctor.DoctorName='"+Dor+"' group by Doctor.DoctorName";
				try {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String Dor=textField.getText();
				String sql="select Doctor.DoctorName, COUNT(charge.RegID) as dn,SUM(charge.sumprice) as money  from Doctor left join charge on Doctor.DoctorID=(select DoctorID from Registration where Registration.RegID=charge.RegID) group by Doctor.DoctorName";
				try {
					ResultSet rs = MDZZ.SqlSel(sql);			
					int count=0;
					while (rs.next()){ //遍历查询结果
